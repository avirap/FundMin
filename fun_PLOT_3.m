...
for b=1:B
            ...
            text(DATES(end)+T/40,PLOT_bench_Return(end,b),['\fontsize{9}',num2str(PLOT_bench_Return(end,b),'%.1f'),'%'],'FontWeight','bold','HorizontalAlignment','left','VerticalAlignment','middle','color',color{b});
Pbench(1,b)=plot(DATES,PLOT_bench_Return(:,b),    'LineWidth',1.25,'color',color{b});
Pbench(2,b)=plot(DATES,PLOT_bench_Return(:,b),'o','LineWidth',0.90,'MarkerFaceColor',color{b},'color','k'); Pbench(2,b).MarkerIndices=EoM;
end
clear color
%% MAIN
...
%%
title(TITLE,'color',[0/255,172/255,0/255],'FontSize',14,'fontweight','bold');
if B<=2; legend([Pmain(1),Pbench(1,:)],LEGEND,'location','northwest','FontSize',14); end
if B>=3; legend([Pmain(1),Pbench(1,:)],LEGEND,'location','northwest','FontSize',6.5); end
